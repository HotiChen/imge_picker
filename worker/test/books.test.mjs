...
test('reading a book returns its stored contents', async () => {
  const body = await (await call(withBook(), '/api/books/b1', {})).json();
  assert.equal(body.name, '婚禮相本');
});

test('an unknown book is a 404', async () => {
  assert.equal((await call(withBook(), '/api/books/nope', {})).status, 404);
});
...
test('approving an album does not block on the notify webhook', async () => {
  ...
  try {
    const res = await Promise.race([
      worker.fetch(req('/api/books/b1/approve', { method: 'POST' }), { imagepicker: bucket }, ctx),
